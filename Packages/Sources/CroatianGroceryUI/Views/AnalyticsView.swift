            ...
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Avg Price")
                        .font(.caption)
                        .foregroundColor(.secondary)
                    Text("€\(String(format: "%.2f", analytics.averagePrice))")
                        .font(.subheadline)
                        .fontWeight(.medium)
                }
                ...
                VStack(alignment: .center, spacing: 4) {
                    Text("Price Range")
                        .font(.caption)
                        .foregroundColor(.secondary)
                    Text("€\(String(format: "%.2f", analytics.minPrice)) - €\(String(format: "%.2f", analytics.maxPrice))")
                        .font(.caption)
                        .fontWeight(.medium)
                }
...
class AnalyticsViewModel: ObservableObject {
    ...
    func loadData() async {
        ...
        // Extract what you need from the MainActor (if manager is isolated to MainActor)
        let manager = dataManager
        
        let result = await Task {
            do {
                let products = try await manager.loadProducts()
                let deals = try await manager.getBestDeals(limit: 5)
                return (products, deals, nil as Error?)
            } catch {
                return ([] as [UnifiedProduct], [] as [UnifiedProduct], error)
            }
        }.value
        
        // Now back to MainActor
    ...
    func refreshData() async {
        ...
        // Extract what you need from MainActor if needed
        let manager = dataManager
